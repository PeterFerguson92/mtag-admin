...
def process_attendance_import(self, request):
    if request.method == "POST":
        ...
        # s3.put_object(Body=xls, Bucket=bucket, Key='tmp' + '/filename.xlsx')
        ...
        #         men_sheet['data'].to_excel(writer)
        #         women_sheet['data'].to_excel(writer)
        #         youth_sheet['data'].to_excel(writer)
        #         children_sheet['data'].to_excel(writer)
        #     data = output.getvalue()
        
        # s3 = boto3.resource('s3')
        # s3.Bucket(bucket).put_object(Key='data.xlsx', Body=data)
...
def retrive_attendance_worksheet_data(xlsx_file, worksheet_name):
    ...
    items = rows[2:]
    return {"date": get_date(rows[0][1]), "items": items, 'data': data}
            ...
            if p[2] == "P" or p[2] == "p":
                ...
                if absentDays > 7:
                    ...
                    Absence.objects.create(
                        member=members[0],
                        contact_phone_number=members[0].telephone,
